...
void CAN1_RX0_IRQHandler(void)
{
	...
	if (CAN_GetITStatus(CAN1,CAN_IT_FMP0)!= RESET)	//判断是否接收结束
        ...
        CAN_Receive(CAN1, CAN_FIFO0, &can1_receive_message);	//获取接收数据
				Can1ReceiveMsgProcess(&can1_receive_message);
...
void CAN2_RX0_IRQHandler(void)
{
	...
	if (CAN_GetITStatus(CAN2,CAN_IT_FMP0)!= RESET)	//判断是否接收结束
        ...
        CAN_Receive(CAN2, CAN_FIFO0, &can2_receive_message);	//获取接收数据
				Can2ReceiveMsgProcess(&can2_receive_message);
	...
	switch(CAN_Set)
	{
		case CAN1_RX:
			switch(Motor_Stdid)
			{
				...
				case 0x209:
					return &ALL_Motor_Msg[16];
				case 0x20A:
					return &ALL_Motor_Msg[17];
			}
			break;
		case CAN2_RX:
			switch(Motor_Stdid)
...
//	CanRxMsg rx_message;
	...
	{
		case 0x201:				
					ALL_Motor_Msg[0].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[0].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[0].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
		      Angle_Count_Long(&ALL_Motor_Msg[0]);
			break;
		case 0x202:				
					ALL_Motor_Msg[1].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[1].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[1].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[1]);		
			break;	
		case 0x203:				
					ALL_Motor_Msg[2].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[2].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[2].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[2]);		
			break;	
		case 0x204:				
					ALL_Motor_Msg[3].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[3].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[3].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[3]);		
			break;	
		case 0x205:	
					ALL_Motor_Msg[4].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[4].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[4].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[4]);		
			break;	
		case 0x206:			
					ALL_Motor_Msg[5].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[5].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[5].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[5]);		
			break;	
		case 0x207:				
					ALL_Motor_Msg[6].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[6].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[6].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[6]);		
			break;	
		case 0x208:				
					ALL_Motor_Msg[7].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[7].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[7].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
          Angle_Count_Long(&ALL_Motor_Msg[7]);		
			break;
		case 0x209:				
					ALL_Motor_Msg[16].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[16].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[16].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;
		case 0x20A:				
					ALL_Motor_Msg[17].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[17].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[17].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;
		case 0x211:				
					Super_C_Msg.Voilt_In=(uint16_t)((can_receive_message->Data[1] << 8) + (can_receive_message->Data[0]));//输入电压
					Super_C_Msg.Voilt_C=(uint16_t)((can_receive_message->Data[3] << 8) + (can_receive_message->Data[2]));//电容电压
					Super_C_Msg.Current_In=(uint16_t)((can_receive_message->Data[5] << 8) + (can_receive_message->Data[4]));//输入电流	
					Super_C_Msg.Target_power=((uint16_t)(can_receive_message->Data[7] << 8) + (can_receive_message->Data[6]));//目标功率	
			break;
...
//	CanRxMsg rx_message;
	...
	{
		case 0x201:				
					ALL_Motor_Msg[8].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[8].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[8].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);
			break;
		case 0x202:				
					ALL_Motor_Msg[9].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[9].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[9].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x203:				
					ALL_Motor_Msg[10].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[10].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[10].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x204:				
					ALL_Motor_Msg[11].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[11].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[11].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x205:	
					ALL_Motor_Msg[12].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[12].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[12].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x206:			
					ALL_Motor_Msg[13].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[13].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[13].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x207:				
					ALL_Motor_Msg[14].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[14].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
					ALL_Motor_Msg[14].dianliu=(can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]);	
			break;	
		case 0x208:				
					ALL_Motor_Msg[15].angle=(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]);//电机1的机械角度值 
					ALL_Motor_Msg[15].speed=(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]);//电机1的速度值
...
{    
    int res1, res2, delta;
	if(ms->angle < ms->Last_Angle){					//angle > last//可能的情况
		res1 = ms->angle + 8192 - ms->Last_Angle;	//正转，delta=+
		res2 = ms->angle - ms->Last_Angle;			//反转	delta=-
	}
	else if(ms->angle == ms->Last_Angle){					//angle > last//可能的情况
		res1 = 0;	//正转，delta=+
		res2 = 0;			//反转	delta=-
	}
	else{	//angle < last
		res1 = ms->angle - 8192 - ms->Last_Angle ;	//反转	delta -
		res2 = ms->angle - ms->Last_Angle;			//正转	delta +
	}
	...
	ms->Angle_Long += delta;//输出偏移量-变化量
	ms->Last_Angle = ms->angle;//更新角度    
}
