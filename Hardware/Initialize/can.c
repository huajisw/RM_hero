...
//	CanRxMsg rx_message;
	switch(can_receive_message->StdId)	//判断标识符（根据电调说明书）
	{
		...
		case 0x20A:				
			...
			break;
		case 0x030:		
					Super_C_Msg.Cap_V=(int16_t)((uint16_t)(can_receive_message->Data[0] << 8) + (can_receive_message->Data[1]));//输入电压
					Super_C_Msg.Cap_I=(int16_t)((uint16_t)(can_receive_message->Data[2] << 8) + (can_receive_message->Data[3]));//电容电压
					Super_C_Msg.Cap_State=(uint16_t)((can_receive_message->Data[4] << 8) + (can_receive_message->Data[5]));//输入电流		
