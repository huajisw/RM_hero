...
fp32 gyro_cali_offset[3] ={-0.012f, 0.016f, 0.0115337158f};
...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        IMU_Cali_Slove(INS_gyro, INS_accel, INS_mag, &mpu6500_real_data, &ist8310_real_data);
				...
				for(int i = 0;i < 3;i++)
				{
					if((INS_gyro[i] > -0.004) && (INS_gyro[i] < 0.004))
					{
							INS_gyro[i] = 0;
					}
				}
...
const fp32* get_JY61_angle_Point(void)
{
		return fAngle;
}

const fp32* get_JY61_Gyro_Data_Point(void)
{
		return fGyro;
}

const fp32* get_JY61_Accel_Data_Point(void)
{
		return fAcc;
}
...
void UART8_IRQHandler(void)
{
	...
	{
		ucTemp = USART_ReceiveData(UART8);
