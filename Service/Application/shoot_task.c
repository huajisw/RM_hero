...
#define Shoot_Key_Read()  GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_10)
...
{
		//电机数据获取
		Data_Init->Fric_Motor_Msg_Get[0] = Get_DJI_Motor_Data(CAN2_RX,FRIC_MOTOR_LEFT_ID); 
		Data_Init->Fric_Motor_Msg_Get[1] = Get_DJI_Motor_Data(CAN2_RX,FRIC_MOTOR_RIGHT_ID); 
...
//		if(Is_Judge_Online())
//		{
//				if(Get_Shoot_Freq->Judge_Shoot_Cooling_Heat < (Get_Shoot_Freq->Judge_Shoot_Cooling_Limit - 110))
...
void Shoot_Control_Data_Set(Shoot_t* Control_Data_Set)
{
		...
		else if(Control_Data_Set->Shoot_Mode == SHOOT_BULLET)
		{
				...
				Control_Data_Set->Trigger_Motor_Angle_Get += (float)(Current_Tick - Control_Data_Set->Trigger_Angle_Timestamp)/1000*((float)Control_Data_Set->Trigger_Motor_Msg_Get->speed/3591*187/60*2*PI);
...
{
		return &Shoot.Trigger_Motor_Current_Send;
}
...
void Shoot_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		CAN2_Motor_Control(SHOOT_MOTOR_ALL_ID,(int16_t)Shoot.Fric_Motor_Current_Send[0],
