...
float Yuntai_Pitch_middle = 2636;
float Yuntai_Pitch_down = 3636;
float Yuntai_Pitch_up = 1636;

float Yuntai_Yaw_left = 5196;
float Yuntai_Yaw_middle = 4086;
float Yuntai_Yaw_right = 2980;


float Yuntai_Pitch_Current_Send = 0;
float Yuntai_Yaw_Current_Send = 0;

float Yuntai_Pitch_Angle_get;
float Yuntai_Yaw_Angle_get;

float Yuntai_Pitch_Angle_set;
float Yuntai_Yaw_Angle_set;

float Yuntai_Pitch_set = 20;
float Yuntai_Yaw_set;
...
//中点设置
#define Gimbal_Yaw_Middle_Angle  -1.60f
...
struct PID yuntai1pid,yuntai2pid;

struct PID Yuntai_Pitch_Angle_Pid,Yuntai_Yaw_Angle_Pid;
struct PID Yuntai_Pitch_Speed_Pid,Yuntai_Yaw_Speed_Pid;

float Yuntai_Pitch_Angle_PidOut,Yuntai_Yaw_Angle_PidOut;
...
#define Gimbal_Yaw_Motor_Angle_Kp 25
#define Gimbal_Yaw_Motor_Angle_Ki 0
#define Gimbal_Yaw_Motor_Angle_Kd 1200
...
#define Gimbal_Pitch_Speed_Kp 1300
#define Gimbal_Pitch_Speed_Ki 2
...
#define Gimbal_Pitch_Motor_Ki 10
...
void Gimbal_Init(Gimbal_t* Gimbal_Init)
{
	...
	//电机数据获取地址
	Gimbal_Init->Gimbal_Yaw_Msg_t.Gimbal_Motor_Msg_Get = Get_DJI_Motor_Data(CAN1_RX,Gimbal_Yaw_Can);
	Gimbal_Init->Gimbal_Pitch_Msg_t.Gimbal_Motor_Msg_Get = Get_DJI_Motor_Data(CAN1_RX,Gimbal_Pitch_Can);
...
{
	Gimbal_Msg_Update->Gimbal_Motor_Angle_Msg = (Gimbal_Msg_Update->Gimbal_Motor_Msg_Get->angle - 4096.00f) /4096.00f * PI;
...
void Gimbal_Mode_Change(Gimbal_t* Mode_Change)
{
	...
	if((Last_Mode_t != Gimbal_Motionless) && (Mode_Change->Gimbal_Mode == Gimbal_Motionless))
	{
		...
		Mode_Change->Gimbal_Pitch_Control_Data = Mode_Change->Gimbal_Pitch_Msg_t.Gimbal_IMU_Angle_Msg;	
	}
...
void Gimbal_IMU_Control_NL_Set(Gimbal_t* Gimbal_Control_Set)
{
	...
	Gimbal_Yaw_Now_Angle = *Gimbal_Control_Set->Gimbal_Yaw_Msg_t.Gimbal_IMU_Angle_Data;
  
	Gimbal_Turn_Head_Flag	 = 0; //调试用
	...
	{
		Gimbal_Yaw_Target_Angle = Angle_Limit(Gimbal_Yaw_Now_Angle + (PI + 2.2)/2);
...
//	Gimbal_Turn_Head_Flag_Last = Gimbal_Turn_Head_Flag;
	...
	Angle_Control_Limit(&Gimbal_Control_Set->Gimbal_Yaw_Add_Data,0.03f,-0.03f);
...
//      Gimbal_Control_Set->Gimbal_Pitch_Add_Data = -(float)((Gimbal_Control_Set->Gimbal_RC_Ctl_Data->rc.ch3)/660.000f * 10) + (float)((Gimbal_Control_Set->Gimbal_RC_Ctl_Data->mouse.y) / Mouse_Y_Set);		
//			//限制每次计算      增加的最大值防止过猛    
//			Angle_Control_Limit(&Gimbal_Control_Set->Gimbal_Pitch_Add_Data,0.04f,-0.04f);
...
//		Gimbal_Control_Set->Gimbal_Pitch_Add_Data =  Gimbal_Control_Set->Gimbal_Pitch_Rise_Add_Data + Gimbal_Control_Set->Gimbal_Pitch_Submit_Add_Data;
    
//		Gimbal_Control_Set->Gimbal_Pitch_Control_Data += Gimbal_Control_Set->Gimbal_Pitch_Add_Data;		
		// 抬头最大距离 以及低头最大距离的限制
		// 距离限制幅值 角度累计的距离 一起判断  缺点是 上电初始时 云台Pitch需要在中间
    ...
    // 修改后的英雄用3508+丝杆控制 所以用 距离环+速度环控制
		...
		if(Gimbal_Control_Set->Gimbal_Pitch_Add_Data == 0)
			Gimbal_Control_Set->Gimbal_Pitch_Control_Data = Gimbal_Control_Set->Gimbal_Pitch_Msg_t.Gimbal_IMU_Angle_Msg;
		else
			Gimbal_Control_Set->Gimbal_Pitch_Control_Data += Gimbal_Control_Set->Gimbal_Pitch_Add_Data;
		...
		Gimbal_Control_Set->Gimbal_Pitch_Msg_t.Gimbal_IMU_Angle_Msg,Gimbal_Control_Set->Gimbal_Pitch_Control_Data);
...
//		Gimbal_Control_Set->Gimbal_Pitch_Control_Data = Gimbal_Control_Set->Gimbal_Pitch_Rise_Add_Data + Gimbal_Control_Set->Gimbal_Pitch_Submit_Add_Data;
...
//		Gimbal_Control_Set->Gimbal_Pitch_Add_Data = -(float)((Gimbal_Control_Set->Gimbal_RC_Ctl_Data->rc.ch3)/660.000f * 10) + (float)((Gimbal_Control_Set->Gimbal_RC_Ctl_Data->mouse.y) / Mouse_Y_Set);
    ...
    J_scope_Distnace = (-Gimbal_Control_Set->Gimbal_Pitch_Msg_t.Motor_Run_Dis_Get_turn_by_angle);
       ...
       Gimbal_Control_Set->Gimbal_Pitch_Rise_Add_Data = 0;
		...
		Gimbal_Control_Set->Gimbal_Pitch_Control_Data += Gimbal_Control_Set->Gimbal_Pitch_Add_Data;
    Gimbal_Control_Set->Gimbal_Pitch_Control_Data = 0;		
		// 抬头最大距离 以及低头最大距离的限制
		// 距离限制幅值 角度累计的距离 一起判断  缺点是 上电初始时 云台Pitch需要在中间
...
void Gimbal_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		Gimbal_3508_Motor_Data_Update(&Gimbal.Gimbal_Pitch_Msg_t);
